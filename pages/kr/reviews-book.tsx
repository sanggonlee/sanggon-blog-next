...
const reviews = [
  {
    ...
    date: '2026.07',
    text: `
       크게 새로운 것을 얻으리란 기대를 하지는 않았지만 생각보다 상당히 생소하고 충격적인 격언들이 많았고 해괴하게 느껴지는 격언들도 있었다.
       생각해보게 만드는 것들임에는 분명했고 삶의 큰 방향을 제시해주는 것들도 분명 있으리라 믿어 의심치 않으나 '왜'를 설명해주지 않음으로써 내게는 조금 힘이 떨어지는 부분들도 많았다.
    `,
    quotes: [
      `나에게는 나의 본성보다 더 신성한 법은 없다. 선과 악은 아주 손쉽게 이것저것에 갖다 붙일 수 있는 이름일 뿐이다. 유일하게 옳은 것은 나의 기질에 맞는 것이고, 유일하게 그른 것은 나의 기질에 어긋나는 것이다.`,
      `당신이 과거에 했던 말과 모순되는 발언을 한다고 치자. 그게 뭐 어떻다는 말인가? 단순히 과거를 떠올릴 때조차 자기기억에만 의존하지 마라. 천 개의 눈이 지켜보는 현재로 과거를 불러내서 재판을 받게 해라. 그렇게 항상 새로운 날을 사는 것이 바로 지혜롭게 사는 길이다.`,
      `내 창문 아래에 피어 있는 장미들은 더 먼저 핀 장미나 더 예쁘게 핀 장미를 언급하지 않는다.`,
      `당신에게 평화를 안겨줄 수 있는 것은 당신뿐이다. 원칙의 승리만이 당신에게 평화를 안겨줄 수 있다.`
    ]
  },
  {
    title: `[B] 사랑의 여러 빛깔`,
    ...
    date: '2026.07',
    text: `
       오랜만에 이문열의 세계명작산책 시리즈를 읽어서 즐거웠다. 이 편에서 사랑이 의미하는 바는 낭만적 사랑, 즉 에로스다.
       <달로 가는 도중에>는, 낭만 그 자체라고 할 수 있다. 그 낭만이 낭비, 혹은 바보같음을 내포한다고 하더라도 말이다.
       <슌킨 이야기>는 너무나도 헌신적인 사랑에 대한 이야기지만 나는 이문열이 왜 이 작품을 그렇게 좋게 평가했는지 알기 어려웠다.
       <임멘 호수>는 특별한 플롯이 없는데도 이루어지지 못한 사랑이 주는 그 아름다움만으로 아련함을 주는 작품이었다.
       <라이젠보그 남작의 운명>은 매정하고 충격적이어서 기억에 남았다.
       <잊힌 결혼식>은 황당하면서도 유쾌했다.
       각 작품에 딸린 이문열의 짧은 해설은 너무 깊이 들어가지 않아서 좋았다.
    `,
    quotes: [
      ``
    ]
  },
  {
    ...
    rating: 3,
    text: `
       SF소설이라고 분류되어 있었는데 별로 SF적인 요소는 없었고 오히려 판타지에 가까웠다.
       플롯을 굉장히 많이 꼬아놓았는데 중반 이후부터는 솔직히 따라가기가 너무 귀찮아서 좀 대충 읽어버렸다.
       처녀작임을 짐작할 수 있는 조금 현학적인 문장들이 많았다.
       좋은 평가를 많이 받은 작품이라고 하는데 나는 큰 인상을 받지는 못했다.
    `,
    quotes: [
      ``
    ]
  },
  {
    ...
    date: '2026.06',
    text: `
       이 책을 읽게 된 것은 신혼여행으로 프랑스 남부에 가게 되었는데 가능하면 여행지에 맞는 작품을 읽고 싶어서였다.
       프랑스 사람 특유의 사고방식과 연애관, 그리고 무엇보다 십대 프랑스 소녀의 섬세한 감성을 술술 읽혀지는 문체로 인해 재밌게 읽을 수 있었다.
       하지만 내가 잘 이해하지 못한건지 이야기의 맥락과 전하고자 하는 바가 모호하고 결말 부분이 너무 갑작스럽게 매듭지어지는 인상을 받았다.
       책 제목인 '슬픔이여 안녕'도 이야기의 맥락과 전혀 맞지 않다가 급작스러운 결말에 갑자기 등장한 듯한 느낌이었다.
    `,
    quotes: [
      `이런 입술, 이렇게 엉망이 된 얼굴, 이 흉하고 제멋대로인 육체적 한계 때문에 나는 나약하고 비겁해질 수 있었던 것일까? 하지만 내가 정말 모자라는 인간이라면, 어째서 그런 나 자신의 모습과는 상반되게 이토록 명료하게 그 사실을 인식하는 것일까? 나 자신을 혐오하는 것, 폭음의 밤을 보낸 대가로 움푹 팬 늑대 같은 내 얼굴을 증오하는 것은 재미있었다.`,
      `변명으로 가득 찬 우리의 편지가 식탁 위에 흩어져 있었다. 나는 한 손으로 편지를 밀었다. 편지지들이 흩날리다가 마루판 위로 떨어졌다. 술을 가득 따른 잔을 들고 돌아오던 아버지는 잠시 주춤거리더니 떨어진 편지를 피해 발을 디뎠다. 그 모든 것이 씁쓸하고 상징적인 의미를 품고 있는 듯했다.`
    ]
  },
  {
    title: `전쟁과 평화 4권`,
    ...
    date: '2026.05',
    text: `
       정말 대단한 작품이다. 작품 전반에 크게 스토리적인 부분과 작가 본인의 의견을 에세이 형식으로 전개하는 부분으로 나뉘는데, 에세이 부분은 굉장히 뚜렷한 역사관과 철학이 드러나며, 이 부분들은 사실 읽기가 정말 힘들었다.
       소설로써는 스토리 부분만 읽어도 크게 지장은 없을 것 같아 너무 읽기 힘든 사람들은 이 부분들은 스킵하고 읽어도 괜찮지 않을까 싶다.
       스토리적인 부분에서는 작품 내 여러 인물들이 성장하는 모습을 보여주며, 특히 피예르, 나타샤, 안드레이 공작이 중심인물로 보여진다.
       이들의 성장은 한 번의 특수한 경험을 통해 얻는 일회적인 깨달음이 다시 옅어지고 색이 바래는 모습들을 보여주면서 그런 깨달음은 신기루적인 성장에 지나지 않음과 동시에 의미가 없는 것도 아닌, 즉 진정한 성장은 껍질을 여러번 탈피하면서 얻어지는 숭고한 과정이라는 것을 일깨워준다.
       높낮이가 많고 호흡이 정말 길어 내가 작품을 제대로 포착하지 못한다는 느낌도 받았지만 매우 세밀한 역사적 묘사들과 매력적인 인물들의 성장과 고난이 맞물리면서 후반부에 가서는 매우 깊은 울림을 주었다. 고전이란 이런 것이다 라고 말해주는 듯하다.
       장장 5개월을 읽은 작품인데 일단 이 3000페이지가 넘는 작품을 다 읽은 시점에서 뜻깊은 감상을 얻었다는 사실이 비교적 만족스럽고 이렇게 긴 호흡의 작품만이 줄 수 있는 특별한 감동이 있다는 것도 느꼈다.
    `,
    quotes: [
      `그러나 인간의 마음속에는 원인을 탐구하려는 욕구가 있다. 그래서 인간의 지혜는 각각이 개별적으로 원인이 될 수 있는 현상들의 수많은 조건과 복잡성은 깊이 탐구하지 않고, 가장 처음의, 가장 알기 쉬운 근접한 것을 포착해 그것을 원인이라 말한다.`,
      `어머니의 마음의 상처는 아물지 않았다. 페탸의 죽음은 그녀의 생명 절반을 앗아갔다. 페탸의 전사 소식을 들은 당시만 해도 생생하고 쾌활한 쉰 살의 여성이었던 그녀가 한 달 뒤 방에서 나왔을 때는 삶과는 아무 관계도 없는 사람처럼 절반은 죽은 듯한 노파가 되어 있었다. 그러나 백작부인을 반쯤 죽인 것과 다름없는 이 새로운 상처가 나타샤를 삶으로 돌아오게 했다. 정신의 몸이 찢겨 생긴 마음의 상처도 육체의 상처와 똑같아서, 그것이 아무리 이상해 보이더라도, 역시 깊은 상처는 아물고 양끝이 붙어 육체의 상처와 마찬가지로 안에서 솟아나는 생명력에 의해 비로소 낫는 법이다. 나타샤의 상처도 역시 그렇게 아물었다. 그녀는 자신의 삶이 끝났다고 생각했었다. 그런데 갑자기 어머니에 대한 사랑이, 그녀에게 삶의 본질인 사랑이 아직 자기 안에 살아 있음을 알게 했다. 사랑이 깨어나고, 삶도 눈을 떴다.`,
      `지금 그는 모든 것 속에서 위대하고 영원하고 무한한 것을 보는 법을 배웠기 때문에 그것을 보기 위해, 그 사색을 즐기기 위해 그때까지 사람들 머리 너머로 들여다보던 망원경을 내던지고, 자기 주위에서 끝없이 변화하는 영원히 위대하고 포착하기 어려운 무한한 삶을 기꺼이 관찰했다. 가까이에서 볼수록 그는 더욱 안정되고 행복감을 느꼈다. 전에 그의 모든 지적인 구축을 파괴했던 왜? 라는 무서운 의문은 더이상 존재하지 않았다. 지금은 왜? 라는 의문에 대해 그의 마음속에는, 신이 있기 때문에, 그의 의지 없이는 머리카락 한 올도 사람의 머리에서 그냥 떨어지지 않는다는 신이 있기 때문이라는 단순한 대답이 준비되어 있었다.`,
      `결혼 후 칠 년이 흘렀을 때 피예르는 자신이 나쁜 사람이 아니라는 것을 기쁜 마음으로 확신하게 되었다. 그는 자기 안에 좋고 나쁜 모든 것이 뒤섞여 있고 하나가 다른 것을 가리고 있다고 생각했었다. 그런데 그는 자기 자신의 반영을 아내에게서 보았고, 그녀에게는 그가 가진 오직 진실로 좋은 것만 반영되어 있었고, 전혀 좋지 못한 모든 것은 버려져 있었다. 그리고 그 반영은 논리적인 사색이 아니라 다른 것에서, 신비하고 직접적인 반영에서 생긴 것이었다.`,
      `우리의 행위가 추상적일수록, 즉 타인의 행위와 결부되지 않을수록 우리는 자유롭고, 반대로 타인과 결부될수록 우리의 행동은 부자유에 가까워진다는 사실을 인정할 수밖에 없다.`,
    ]
  },
  {
    title: `전쟁과 평화 3권`,
    ...
    date: '2026.04',
    text: `
       톨스토이가 역사학자가 아닌가 싶을 정도로 굉장히 역사에 관해 뚜렷한 철학을 가지고 있는 것으로 보인다.
       피예르와 나타샤, 니콜라이 로스토프 등 캐릭터가 무르익을 대로 무르익어 너무나도 매력적이었지만, 도리어 이제와서 이 작품은 호흡이 너무 길어 읽기가 어렵다고 느꼈다.
       이야기를 따라가는 것이야 그냥 읽으면 그만이지만, 전체 맥락을 총체적으로 파악하는 것은 정말 쉽지 않다고 느꼈던 것이다.
    `,
    quotes: [
      `그들은 두려워하고, 허영에 차고, 기뻐하고, 분개하고, 생각하고 판단하면서 스스로 자신이 무엇을 하고 있는지 알고 또 그것이 자신을 위한 거라고 생각했지만, 사실은 그들 모두가 의지를 갖지 않는 역사의 도구였으며, 그들에게는 보이지 않았지만 우리에게는 이해가 될 일을 하고 있었다. 그것이 실제로 활동하는 모든 인간에게 주어지는 불변의 운명이고, 인간 사회에서 계급이 높을수록 자유는 줄어든다.`,
      `위험이 닥쳐오면 인간의 마음속에서는 으레 두 개의 목소리가 똑같이 강하게 말하기 시작하는데, 하나의 목소리는 위험의 성질을 잘 파악해 벗어날 수단을 강구해야 한다고 무척 이성적으로 말하고, 또하나의 목소리는 모든 것을 예견하고 사건의 전반적인 움직임에서 달아나는 것은 인간의 힘에 부치고 위험을 생각하는 것은 괴롭고 고통스러우니 그것이 눈앞에 닥칠 때까지는 외면하고 즐거운 일만 생각하는 편이 현명하다고 더욱 이성적으로 말한다. 혼자일 때 인간은 대개 첫번째 목소리에 따르지만, 집단사회는 두번째 목소리에 따른다.`
    ]
  },
  {
    title: `전쟁과 평화 2권`,
    ...
    date: '2026.03',
    text: `
       한 사람이 어떻게 이렇게 인간 삶의 군상과 심리를 꿰뚫어 볼 수 있는지 경이로울 따름이었다.
       피예르가 프리메이슨에 가입하는 내용도 흥미로웠고 안드레이 공작이 은둔하며 지내다가 떡갈나무를 보고 다시 의욕을 찾는 모습은 원효대사의 해골물을 연상케 했다.
       순수한 나타샤가 안드레이 공작과 아나톨을 거쳐가며 사랑에 대해 깨닫고 성장해가는 모습도 가슴 졸이고 안타까우면서도 희망을 보여주었다.
    `,
    quotes: [
      ...
      `오히려 피예르는 눈물에 젖은 눈으로 행복하게 그 밝은 별을 바라보았고, 형언할 수 없을 만큼 빠른 속도로 포물선을 그리며 무한한 공간을 날던 별은 갑자기 대지에 똑바로 꽂힌 화살처럼, 검은 밤하늘의 자신이 선택한 어느 한 점에 뛰어들어 힘차게 꼬리를 치켜세우고 수없이 반짝이는 다른 별들 속에서 하얀빛을 튀기고 반짝이며 멈췄다. 피예르는 그 별이 새로운 생활을 향해 활짝 꽃펴 부드럽고 고무된 그의 영혼 속에 있는 무언가에 화답해 주고 있는 것 같았다.`
    ]
  },
  {
    title: `전쟁과 평화 1권`,
    ...
    date: '2026.01',
    text: `
       몇백 페이지가 넘어갈 때까지도 등장인물이 너무나도 많아 어질어질했고 생소한 당대 러시아 귀족문화에 몰입하기 힘들었지만
       당시 나폴레옹과의 전쟁과 러시아 귀족사회에 깊숙이 들어와 있던 프랑스 문화를 향한 노골적인 동경을 느낄 수 있었다.
       아우스터리츠 전투에서 안드레이 공작이 전장에 누워 명예와 영광의 덧없음을 느끼는 장면이 인상깊었다.
       굉장히 많은 등장인물이 저마다의 이야기로 삶에 관해 나름의 깊은 통찰을 보여주기에 정말로 대작이라는 생각이 들었다.
    `,
    quotes: [
      `"어이, 자네 아나?" 그는 말했다. "사랑을 하지 않는 동안은 자고 있는 거나 다름없어. 우리는 먼지의 자식이야...... 그러나 사랑을 하면 하느님이 되지. 창조의 첫날처럼 깨끗해져......"`,
      `'어쩌면 이렇게도 조용하고 평온하고 엄숙할까. 내가 달리던 때와는 전혀 다르다.' 안드레이 공작은 생각했다. '우리가 달리고 외치고 싸우던 때와는 전혀 다르다. 저 프랑스병과 포수가 적의에 불타고 공포에 질린 얼굴로 서로 세간을 잡아당기던 때와는 전혀 다르다. 이 드높고 끝없는 하늘에 흘러가는 구름은 전혀 다르다. 왜 나는 전에 이 드높은 하늘을 보지 못했을까? 그러나 이제라도 깨달았으니 나는 얼마나 행복한가. 그렇다! 모두 허무하다, 모두 거짓이다, 이 끝없는 하늘 외에는. 그러나 이 하늘마저도 없다, 아무것도 없다, 정적과 평안 외에는. 그것으로 좋은 것이다!......'`
    ]
  },
  {
    title: `소문`,
    ...
    rating: 3,
    text: `
       마케팅 문구에 혹해서 읽은 책이다. 오랜만에 정말 충격적인 반전을 경험해보고 싶었기 때문이다.
       그 부분에 있어서는 조금 실망스러웠다. 작품 전체를 관통하는, 뒷통수가 얼얼할 정도의 반전을 기대했는데 너무 뻔한 작품이 되지 않으려고 일부러 삽입한 듯한 느낌이었다.
       그 외에는 추리소설로써는 그냥 무난했다. 일본 내에서의 경찰들의 계급구조와 현실을 적나라하게 보여주는 점은 나름 흥미로웠다.
    `,
    quotes: [
      `쓰에무라의 머릿속에 고향에서 보았던 누에 치는 오두막이 떠올랐다. 뽕잎을 깐 시렁 위에 수많은 누에가 꿈틀거리는 모습이다. 어두컴컴한 가운데 누에들이 탐욕스럽게 뽕잎을 먹는 소리는 서로 존재도 제대로 알지 못하면서 속삭이는 목소리처럼 들렸다.`
    ]
  },
  {
    title: `[J] 아직도 가야 할 길`,
    ...
    rating: 3.5,
    text: `
       사람은 어떻게 살아야 하는가, 라는 것에 답을 하는 부류의 책이라고 볼 수 있다.
       1부 <훈련>과 2부 <사랑> 부분은 수긍되고 새로 얻는 것들이 많았다. 사랑에 대한 나름의 정의와 정신적 성숙이 우리 삶의 방향이 되어야 한다는 점은 일면 새롭게 삶을 보게 했다.
       3부와 4부에서 종교와 신비에 관해 논할 때는 비약이 너무나도 많았다. 뿐만 아니라 아주 얕은 과학 지식에서 온 잘못된 정보 (엔트로피)를 근거로 삼는데 그 부분에서 신뢰를 꽤 잃어버렸다.
       어쨌거나 진정한 사랑을 이루는 것은 쉽지 않음을 이 책은 분명히 하고 있고, 결혼을 준비하면서 읽은 다른 두 책과 사랑이 무엇인지에 관해 맥을 함께 한다.
    `,
    quotes: [
      `...다소 부적절하게 보일지는 모르나 사랑을 간단히 다음과 같이 정의하고자 한다. '자기 자신이나 타인의 영적 성장을 돕기 위해 자신을 확대하려는 의지'라고 말이다.`
    ]
  },
  {
    title: `희랍어 시간`,
    ...
    rating: 4.5,
    text: `
       마치 한 편의 긴 시를 읽는 것 같았다. 섬세함의 극이라고 불러도 손색이 없을 것이다.
       감성적이고 서정적인 문장들을 마주칠 때마다 하이라이트하다가 나중에는 하이라이트하지 않은 줄을 찾기 어려울 정도였다.
       절대 쉽게 읽히는 책은 아니지만, 두고두고 읽으며 참고하고 싶은 책이다.
    `,
    quotes: [
      `문장과 문장 사이에 긴 사이를 둔다. 어두운 곳에서 글을 쓸 때, 윗문장에 아랫문장을 겹쳐 쓰지 않으려고 가능한 한 넓게 간격을 두는 것처럼.`,
  ...
  },
  {
    ...
    rating: 3.5,
    text: `
       책들이 질문을 던지는 책과 대답을 하는 책으로 나뉜다고 한다면, 이 책은 단연코 질문을 던지는 쪽이다.
       그 질문 자체도 그렇고, 이야기 자체도 그렇고 어떤 사람들은 이 작품을 뼛속까지 혐오하리라.
       하지만 책에서 일컫는 '표백 세대'의 일원으로서 나는 이 소설이 던지는 질문에 분명히 날카로운 면이 있다고 생각한다.
       초반에는 미스터리 서사로 흥미진진했고 갈수록 우울하고 독자의 기분을 잠식하는 내용이다. 재밌게 읽었다.
    `,
  },
  {
    title: `대온실 수리 보고서`,
    ...
    rating: 3.5,
    text: `
       여러 시간대와 역사적 사실, 개인의 상처 등 여러 요소들이 꽤나 정교하고 복잡하게 맞물린 구성을 보여준 작품.
       문자 할머니의 과거, 대온실 지하의 인골, 그리고 화자 영두의 상처를 발굴할 때의 평행성이 흥미로웠고
       일본 패망 직후 한국에 살았던 소수약자로서의 일본인은 사실 한번도 생각해보지 못한 관점이어서 굉장히 신선했다.
       상처라는 것을 대할 때의 우리의 자세를 말하고 있고 작가는 이걸 훌륭하게 해낸 듯하다.
    `,
  },
  {
  ...
  },
  {
    ...
    text: `
      ...
      들어서 어느 정도 알고 있는 내용이긴 했지만 이런 책은 살면서 미처 돌아보지 못했던 것들, 그리고 방향성을 알려준다. 다만 또 금방 잊어버리게 되는것도 사실이다.
